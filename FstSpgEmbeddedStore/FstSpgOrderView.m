...
@implementation FstSpgOrderView
...
- (void)layout
{
	if([[self subviews] count] == 0) {
		...
		FstSpgOrderDocumentRepresentation *representation = [[self dataSource] representation];
		FstSpgOrder *order = [representation order];

		FstSpgEmbeddedStoreController *delegate = [[[[self dataSource] webFrame] webView] UIDelegate];
