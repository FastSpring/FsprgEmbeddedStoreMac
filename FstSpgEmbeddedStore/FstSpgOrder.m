...
+ (FstSpgOrder *)orderFromData:(NSData *)aData
{
	...
	return [[[FstSpgOrder alloc] initWithDictionary:aDict] autorelease];
}
...
- (FstSpgOrderItem *)firstOrderItem
{
	...
	return [FstSpgOrderItem itemWithDictionary:[items objectAtIndex:0]];
}
...
- (NSArray *)orderItems
{
	...
	for (i = 0; i < count; i++) {
		...
		[orderItems addObject:[FstSpgOrderItem itemWithDictionary:anItem]];
	}
